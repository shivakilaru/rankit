...
createLegend(leg);if(arr.legend){$(el).append("<div id='legendHolder"+unique+"'></div>");$('#legendHolder'+unique).css({'width':legendWidth,'float':'right','text-align':'left'});$('#legendHolder'+unique).append(legend);$('.legendBar'+el.id).css({'float':'left','margin':3,'height':30,'width':30,'font-size':0});}
if(arr.title){$(el).wrap("<div id='graphHolder"+unique+"'></div>");$('#graphHolder'+unique).prepend(arr.title).css({'width':arr.width+'px','text-align':'center'});}};createLegend=function(legendArr){legend='';for(var val in legendArr){legend+="<div id='legend"+legendArr[legendArr.length-1-val][3]+"' style='white-space: nowrap; overflow: hidden; text-overflow: ellipsis; zoom: 1;'>";legend+="<div class='legendBar"+legendArr[legendArr.length-1-val][2]+"' id='legendColor"+legendArr[legendArr.length-1-val][3]+"' style='background-color:"+legendArr[legendArr.length-1-val][0]+"'></div>";legend+="<div class='legendLabel"+legendArr[legendArr.length-1-val][2]+"' id='graphLabel"+unique+"'>"+legendArr[legendArr.length-1-val][1]+"</div>";legend+="</div>";}};this.each(function()
{init(this);})};$.fn.jqBarGraph.defaults={barSpace:10,width:400,height:300,color:'#000000',colors:false,lbl:'',sort:false,position:'bottom',prefix:'',postfix:'',animate:true,speed:0.7,legendWidth:100,legend:false,legends:false,type:false,showValues:true,showValuesColor:'#fff',title:false};function sortNumberAsc(a,b){if(a[0]<b[0])return-1;if(a[0]>b[0])return 1;return 0;}
